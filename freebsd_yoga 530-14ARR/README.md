# FreeBSD on Lenovo Yoga 530

Install scripts to get a working XFCE desktop on FreeBSD, covering the two
Yoga 530 chassis variants and their CPU/GPU configurations:

- **Yoga 530-14IKB** — Intel (Kaby Lake / Kaby Lake-R + optional Nvidia MX130)
- **Yoga 530-14ARR** — AMD (Raven Ridge Ryzen + Vega iGPU, no dGPU option)

## Which script do I run?

### Yoga 530-14IKB (Intel)
...
| CPU | iGPU | Script |
|---|---|---|
| Ryzen 3 2200U | Radeon Vega 3 | `install_xfce_ryzen3_2200u.sh` |
| Ryzen 5 2500U | Radeon Vega 8 | `install_xfce_ryzen5_2500u.sh` |

The 14ARR chassis has no dGPU option — both configs run on the Vega iGPU via
`amdgpu`, so there's no hybrid-graphics caveat here like on the 14IKB/MX130.

## Usage

```sh
git clone https://github.com/stuffbymax/freebsd-lenovo-devices.git
cd freebsd-lenovo-devices/yoga-530   # adjust to wherever these scripts live in the repo
chmod +x *.sh
./install_xfce_<your_model>.sh
```

`common.sh` (and `nvidia_mx130.sh` for the MX130 variants) must stay in the
same folder as the model script you're running — each model script sources
them rather than duplicating the logic.

## What the scripts do

1. Update the base system and packages (`freebsd-update`, `pkg upgrade`)
2. Install `drm-kmod` + Xorg, and enable the right KMS driver:
   - 14IKB: Intel `i915kms`
   - 14ARR: AMD `amdgpu`
...
6. Enable CPU power management:
   - 14IKB: `powerdxx` (Intel Speedstep/EST)
   - 14ARR: `powerd` (generic `cpufreq(4)`, since powerdxx is Intel-only)
...
- PRs welcome if you've got a Yoga 530 variant (14IKB or 14ARR) not covered here.
